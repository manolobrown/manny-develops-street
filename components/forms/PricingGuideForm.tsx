...
export function PricingGuideForm() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("/api/newsletter", {
        ...
        body: JSON.stringify({ email, form: "pricing-guide" }),
      });
    ...
    return (
      <div className="flex flex-col gap-2">
        ...
        <span className="font-mono text-[10.5px] uppercase tracking-[0.14em] text-ink-3">
          PDF arrives right after you click the confirm button.
        </span>
      </div>
    );
  }
    ...
    <form onSubmit={onSubmit} className="flex flex-col gap-3">
      <div className="flex overflow-hidden rounded-full border border-line">
        ...
        <button
          ...
          "
        >
          {state === "sending" ? "Sending…" : state === "error" ? "Try again" : "Send the guide ↗"}
        </button>
      </div>
      <span className="font-mono text-[10.5px] uppercase tracking-[0.14em] text-ink-3">
        One PDF · No follow-up sequence · Unsubscribe at the bottom of every email
      </span>
    </form>
  );
}
